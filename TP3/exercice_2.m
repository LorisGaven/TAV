...
function hist = ksdensity(I, liste_nvg)
    hist = zeros(length(liste_nvg), 1);
    for i = 1:length(I)
        value = min(round(I(i)), liste_nvg(end));
...
function [poids,argument] = argument_eq_8(mu_test,sigma_test,liste_nvg,F)
    x = repmat(liste_nvg', 1, length(mu_test));
    A = (1./(sigma_test*sqrt(2*pi))).*exp(-(x-mu_test).^2./(2*sigma_test.^2));
    poids = A\F;
    argument = sum(F - sum((poids'./(sigma_test*sqrt(2*pi))).*exp(-(x-mu_test).^2./(2*sigma_test.^2)),2));
end