...
function u_kp1 = calcul_structure_3(u_k, u, Dx, Dy, Phi, epsilon, mu_prime, gamma)
    
    [h, w] = size(u_k);

    p1 = ifft2(Phi .* (fft2(u_k) - fft2(u)));
    p1 = real(p1(:));
    ...
    p2 = (u_xx .* (u_y.^2 + epsilon) + u_yy .* (u_x.^2 + epsilon) - 2 .* u_x .* u_y .* u_xy) ./ (u_x.^2 + u_y.^2 + epsilon).^(3.0/2.0);
    u_kp1 = reshape(u_k - gamma .* (p1 - mu_prime .* p2), [h,w]);
end
