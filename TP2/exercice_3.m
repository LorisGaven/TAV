...
function mv = max_vraisemblance_2(D_app, param, sigma)
    nb_param = length(param);
    l = zeros(nb_param,1);
    for i = 1:nb_param
        l(i) = sum(log( (0.5/sigma) * exp(-calcul_r(D_app, param(i,1,:)).^2 ./ (2*sigma)^2) + (0.5/sigma) * exp(-calcul_r(D_app, param(i,2,:)).^2 ./ (2*sigma)^2)));
    end
    [~, i_max] = max(l);
    mv = param(i_max,:,:);
end

function p = probabilites(D_app,param,sigma)
    p = [0.5/(sigma*sqrt(2*pi))*exp(calcul_r(D_app, param(1,:)).^2 / (2*sigma^2)); 0.5/(sigma*sqrt(2*pi))*exp(calcul_r(D_app, param(2,:)).^2 / (2*sigma^2))];
end
