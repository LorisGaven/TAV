...
D = imread('Images/masque_randonneur.png');
...
% Point fixe :
lambda = 0.1;			% Poids de la regularisation
u_k = u_0;
convergence = +Inf;
iteration = 0;
...
while convergence > 1e-3
	...
	% Iteration (6) :
	u_kp1 = impainting(b,u_k,lambda,Dx,Dy,epsilon,D);	
...
function u_kp1 = impainting(u_0,u_k,lambda,Dx,Dy,epsilon,D)
        n = length(u_k);
        d_u_x = Dx*u_k;
        d_u_y = Dy*u_k;
        V = 1 ./ sqrt(d_u_x.^2 + d_u_y.^2 + epsilon);
        W = spdiags(V, 0, n, n);
        Wd = spdiags(1 - D(:), 0, n, n);
